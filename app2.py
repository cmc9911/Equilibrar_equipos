...
def mostrar_inicio():
    ...
    if st.session_state.scene == "inicio":
        ...
        st.button("👥 Empezar selección de jugadores", on_click=lambda: seleccion())
        st.button("📂 Cargar nuevo archivo", on_click=lambda: cambiar_escena("cargar"))
        
def seleccion():
        if st.session_state.data is None:
            st.warning("⚠️ Primero debes cargar un archivo Excel antes de continuar.")
        else:
            cambiar_escena("seleccion")


# ---------- ESCENA 2: CARGAR ARCHIVO ----------
def mostrar_carga():
    st.title("📤 Cargar nuevo archivo Excel")
...
# ---------- ESCENA 3: SELECCIÓN DE JUGADORES ----------
...
def mostrar_resultados():
    ...
    #     for j in equipo1:
    #         st.write(f"{j['Nombre del jugador']} - Nivel: {j['Nivel general (1 a 10)']:.1f}")
    ...
    # st.write(f"📉 Diferencia total: **{diferencia:.2f}**")
    # st.button("⬅ Volver al inicio", on_click=lambda: cambiar_escena("inicio"))
    ...
    campo_base64 = get_base64_image("campofutbol.jpg")  # imagen 1 sola vez, luego se usa 3 veces

    # FUNCIÓN PARA MOSTRAR CAMPO + EQUIPOS
    def dibujar_enfrentamiento(jugadores):
        jugador_base64 = get_base64_image("jugador.png")
